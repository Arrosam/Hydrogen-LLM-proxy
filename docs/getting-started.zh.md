...
保存之前，先点弹窗底部的**「测试」**：它会用你刚填的这套配置——基础 URL、密钥、额外请求头——去调该提供商的模型列表接口。所以在任何东西被存下来**之前**，你就知道这个提供商通不通了。返回的是该提供商自己的模型清单，当场显示在弹窗里，并在你点**保存**时一并写入数据库。

存下来的这份清单，正是让第 2 步的**上游模型 ID** 从「凭记忆手打」变成「从列表里挑」的东西。上游新增或下架模型时，重新打开该提供商再点一次「测试」即可——每次测试都会整份替换旧清单。

保存后提供商会出现在列表里。**密钥**列只显示「已设置」，永远不显示密钥本身；**模型**列显示上次测试拿到了多少个模型。

![提供商列表，每行有类型、基础 URL、密钥状态和模型数量](images/zh/01-providers.png)

> **以后再改时：** API 密钥输入框每次打开都是空的。留空表示保持原密钥不变，只有真要换密钥时才填新的。留空时做测试会自动复用已存的密钥，所以重测一个已保存的提供商不用重新输入任何东西。
...
**模型映射 → 新建模型。** 起一个简短、稳定的名字——`sonnet4.6`、`gpt5.4`、`fast-local`。这是**你自己的**名字；上游那串 ID 稍后在映射里填。描述可选。
...
- **上游模型 ID** — 该提供商在通信时期望的精确字符串，如 `claude-sonnet-4-6` 或 `gpt-4o`。这里写错不会当场报错，而是要等到真正调用时才暴露成上游 404。
  ...
  如果第 1 步里测试过这个提供商，它报告的全部模型会列在输入框下方：输入即筛选，点击即填入。从这份列表里挑，正是避开那个 404 的办法——它是提供商自己的答案，不是你的记忆。当然也不强制：列表里没有的 ID 照样可以手打。看到「该提供商暂无模型列表」，就回「提供商」页跑一次测试。
...
**API 密钥 → 签发 API 密钥。** 仅管理员可签发，manager 不行。

- **名称** — 如 `my-laptop`。必填。
- **范围** — 默认勾着**允许全部模型服务**。取消勾选后可以只勾特定服务——想给别人一把「只能用便宜那个」的钥匙，就靠它。
- **最大请求数（可选）/ 最大 token 数（可选）** — 这个密钥的配额。
- **过期时间（可选）**。

密钥以 `sk-hproxy-...` 的形式**只显示一次**。请立刻复制：服务器只存哈希，所以之后谁也找不回来，**包括你自己**。丢了就重新签发一个。
...
点**校验**，再**保存**。给它签发一个 API 密钥（或者用「允许全部模型服务」的密钥），然后直接按名字调用：
...
**描述会被缓存。** 每张图片以自身内容的哈希为键，因此同一张图片只转写一次；之后不管是同一轮对话的后续、另一个请求，还是另一个客户端再送过来，都直接命中缓存，不会再打模型。一个同时含新图和旧图的请求，只会把新图发出去。缓存受一个字节上限约束，在**设置 → OCR 图片缓存**里配置（默认 64 MiB）；超出上限时会按最久未使用的顺序删除，直到重新落回上限之内。把上限设为 0 即关闭缓存并清空。
...
| 你看到的 | 意味着什么 |
|---|---|
| 步骤的提供商下拉框里是`（无已映射提供商）` | 这个模型还没做映射。去 模型映射 → **映射提供商**。 |
