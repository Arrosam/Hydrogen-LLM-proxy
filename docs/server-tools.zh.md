...
工具循环到达轮次上限时**不报错**，而是返回 `stop_reason: "pause_turn"`，保留模型这一轮已经说出的内容。客户端**把该响应原样发回**即可继续，直到循环自然结束。

```
第一轮请求 → stop_reason: "pause_turn"
              content: [ 已有文字, server_tool_use（尚未执行的调用） ]
客户端把 content 原样放回 assistant 消息 → 代理执行该调用并继续
```

要点：

- **暂停发生在调用发出之前**，所以没有任何“执行到一半”的工具；续跑时整批重放是干净的。
- 暂停时**不发送结果块**。`server_tool_use` 之后没有结果，正是“此调用待执行”的协议形态；为它编造一个结果会报告一次并未发生的失败。
- **调用次数预算（`maxCalls`）不走暂停**：它按**每个调用**判定，超出的调用以 `tool_call_limit` 错误返回给模型，模型当轮即可调整；只有**轮次上限**才结束整个回合。
- 续跑需要该服务的调用预算仍有余额；预算耗尽而无法继续时会明确报错，而不是静默半执行。
- Responses 侧没有 `pause_turn` 状态：对应形态是 `status: "incomplete"` 且 `incomplete_details.reason = "pause_turn"`。
