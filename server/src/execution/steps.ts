import type { AdvanceTrigger, ServiceStep, ServiceSteps, Trigger } from "./definition";
...
export async function runSteps<T>(
  ...
  for (let i = 0; i < steps.steps.length; i++) {
    ...
    for (let a = 1; a <= maxAttempts; a++) {
      ...
      if (res.ok) {
        path.push({ step: i + 1, attempt: a, model: step.model, provider: step.provider, status: 200, kind: "ok", latencyMs });
