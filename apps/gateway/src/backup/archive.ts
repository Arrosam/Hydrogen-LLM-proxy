...
import { STATS_CACHE_SETTINGS_KEY } from "../persistence/statsCache.js";
import { decryptSecret, encryptSecret } from "@areelai/common";
import { decryptProviderKey, encryptProviderKey, SUPPLIER_TABLES } from "@areelai/supplier-management";
...
import { GATEWAY_TABLES } from "../db/schema.js";
import { openWithPassphrase, sealWithPassphrase, type SealedPayload } from "../security/passphrase.js";
 ...
 */
const TABLES = [
  // Each package names its own tables, parents before children. Nothing
  // references across a package line, so only the order within one matters.
  ...USER_TABLES,
  ...SUPPLIER_TABLES,
