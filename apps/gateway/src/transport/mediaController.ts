...
import { buildErrorBody, extractUpstreamMessage, failureMessage, failureStatus, type Usage } from "@areelai/wire-format";
import {
  buildHeaders,
  providerEndpoints,
  servesOpenAiMedia,
...
import {
  ...
  parseVideoId,
  readMultipartField,
  rewriteMultipartField,
  runMediaSteps,
  serviceCategory,
  suffixVideoId,
  upsertMultipartField,
  zeroUsage,
  type AttemptResult,
  type MediaCategory,
  type MediaHit,
...
import { tokenAllowsService } from "@areelai/user-management";
import { requireClientToken } from "../auth/tokenAuth.js";
import { genId } from "@areelai/common";
import type { ModelServiceRow, Token } from "../db/schema.js";
import type { HttpRequestInfo } from "../observability/requestLogger.js";
 ...
 * override parameters), and the step chain's retry/fallback rules apply. The
 * execution half (endpoint selection, video id routing, usage) lives in
 * model-services; this file reads the client request and writes the reply.
 */
...
export class MediaController {
  ...
  private loadService(
    ...
    if (!isStepChain(def)) {
      this.replyError(reply, 400, `'${serviceName}' is a ${def.kind === "micro_agent" ? "Micro Agent" : def.kind}; this endpoint serves ${category} Model Services only.`);
      return null;
    }
    const actual = serviceCategory(def);
    if (actual !== category) {
      // chat AND ocr services live on the chat endpoints.
      const hint = actual === "chat" || actual === "ocr" ? "/v1/chat/completions" : MEDIA_ENDPOINT_BY_CATEGORY[actual as MediaCategory];
  ...
  private run(
    ...
    return runMediaSteps(this.deps.catalog, def, category, signal, send);
  }
  ...
  private async handleJson(req: FastifyRequest, reply: FastifyReply, category: MediaCategory): Promise<unknown> {
    ...
    const outcome = await this.run(def, category, signal, async (step, target) => {
      const upstreamBody = { ...body, ...mediaStepParams(step), model: target.upstreamModel };
    ...
    this.record(ctx, outcome, hit.status, mediaUsage(category, json), hit.sentBody);
  ...
  private async handleTranscription(req: FastifyRequest, reply: FastifyReply): Promise<unknown> {
    ...
    const outcome = await this.run(def, "stt", signal, async (step, target) => {
      ...
      let form = raw;
      for (const [k, v] of Object.entries(mediaStepParams(step))) {
        if (v == null) continue;
  ...
  private async handleVideoGet(req: FastifyRequest, reply: FastifyReply, content: boolean): Promise<unknown> {
    ...
    try {
      ...
      if (isStepChain(def)) timeoutMs = def.timeoutMs;
