...
import { idParam, parse } from "@areelai/common";
import { asMillis } from "@areelai/common";
import type { Container } from "../composition/container.js";
import { requireSession } from "../auth/middleware.js";
import { keyStatus, SESSION_COOKIE, type SessionPayload } from "@areelai/user-management";
import { DEFAULT_ADMIN_PASSWORD } from "@areelai/user-management";
import { isChatPipeline, isStepChain, serviceCategory, summarizeService, type ServiceDef } from "@areelai/model-services";
import { ServiceValidationError } from "@areelai/model-services";
import { AgentOcrSchema, buildOcrRequest, isAgent, parseOcrResults, type AgentOcr } from "@areelai/micro-agent";
import type { ModelService } from "@areelai/model-services";
import { buildRequest } from "@areelai/wire-format";
import { textOf, type ImagePart } from "@areelai/wire-format";
import { failureMessage } from "@areelai/wire-format";
import { discoverModels, MAX_DISCOVERED_MODELS, MAX_MODEL_ID_LENGTH } from "@areelai/supplier-management";
import { BLOCK_THRESHOLD_MS } from "../observability/activeRequests.js";
import { withJsonHeartbeat } from "./jsonKeepalive.js";
import { benchRoutes } from "./benchRoutes.js";
import { proxyRoutes } from "./proxyRoutes.js";
import { hostedToolRoutes } from "./hostedToolRoutes.js";
import { BackupError, exportBackup, restoreBackup } from "../backup/archive.js";
import { PassphraseError } from "../security/passphrase.js";
import { APP_VERSION } from "../util/version.js";
import type { ModelServiceRow } from "../db/schema.js";
...
export async function adminRoutes(app: FastifyInstance, c: Container): Promise<void> {
  ...
  const sessionGuard = requireSession(c.users, c.sessions, () => c.settings.sessionEpochMs());
...
async function authRoutes(app: FastifyInstance, c: Container, sessionGuard: ReturnType<typeof requireSession>): Promise<void> {
  ...
  app.post("/login", { config: { rateLimit: { max: 10, timeWindow: "1 minute" } } }, async (req, reply) => {
    ...
    const token = c.sessions.sign({ uid: user.id, username: user.username, role: user.role });
    reply.setCookie(SESSION_COOKIE, token, c.sessions.cookieOptions(c.sessions.resolveCookieSecure(req.protocol === "https")));
    // The same token comes back in the body: a script presents it as
    // `Authorization: Bearer` instead of keeping a cookie jar.
    return { user: c.users.toPublic(user), token };
  });
...
async function checkRoutes(app: FastifyInstance, c: Container): Promise<void> {
  app.post("/check", { config: { rateLimit: { max: 30, timeWindow: "1 minute" } } }, async (req, reply) => {
    ...
    return { key: c.tokens.toPublic(token), status: keyStatus(token) };
  });
}
...
async function serviceRoutes(app: FastifyInstance, c: Container): Promise<void> {
  ...
  app.post("/validate", async (req, reply) => {
    ...
    try {
      ...
      const kind = isStepChain(def) ? "resilience" : "agent";
      const count = isStepChain(def) ? def.steps.length : isAgent(def) ? def.stages.length : 0;
  ...
  app.post("/", async (req, reply) => {
    ...
    try {
      ...
      const row = c.sqlite.transaction(() => {
  ...
  app.post("/test", async (req, reply) => {
    ...
    const request = buildRequest("openai_completion", {
      ...
      params: { maxTokens: isStepChain(def) ? 16 : 64 },
      stream: false,
    });
...
async function tokenRoutes(app: FastifyInstance, c: Container): Promise<void> {
  ...
  app.delete("/:id", async (req, reply) => {
    ...
    if (!id) return reply.code(400).send({ error: "invalid id" });
    if (!c.tokens.get(id)) return reply.code(404).send({ error: "not found" });
    // The key's stored responses and conversations go with it. They live in
    // the gateway's tables and reference the key by id, so the cascade that
    // used to be a foreign key is an explicit call here.
    c.sqlite.transaction(() => {
      c.responses.deleteForToken(id);
      c.tokens.delete(id);
    })();
    return { ok: true };
  });
...
function serializeActive(r: import("../observability/activeRequests.js").ActiveRequest, now: number) {
