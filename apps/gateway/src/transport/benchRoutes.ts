...
import { z } from "zod";
import { parse } from "@areelai/common";
import type { Container } from "../composition/container.js";
import { buildRequest, parseRequest, serializeStream } from "@areelai/wire-format";
import type { Family } from "@areelai/wire-format";
import { MEDIA_FAMILIES, type ResolvedTarget } from "@areelai/supplier-management";
import {
  buildHeaders,
  embeddingsUrl,
  imagesUrl,
...
import { extractUpstreamMessage, failureMessage, failureStatus } from "@areelai/wire-format";
import {
  isStepChain,
  isChatPipeline,
  parseService,
...
import { withThinkingFormat, type ThinkingFormat } from "@areelai/wire-format";
import { classifyError, relayStream, sendBuffered, type AttemptRecord, type AttemptResult } from "@areelai/model-services";
import type { Request as CanonicalRequest } from "@areelai/wire-format";
import { newAccumulator, tapStream } from "@areelai/wire-format";
  ...
  app.get("/targets", async () => {
    const services = c.services.list().map((row) => {
      ...
      try {
        ...
        kind = isStepChain(def) ? "model_service" : "micro_agent";
      } catch {
        valid = false;
      }
...
interface ChatValue {
  response: import("@areelai/wire-format").Response;
  served: Served;
...
async function runChat(
  ...
  const sent = await sendBuffered(egress, c.transport, {
...
async function runChatStream(
  ...
  try {
    ...
    const t = res.target;
    const egress = buildRequest(t.family, request.data());
    const relayed = await relayStream(egress, c.transport, {
      upstreamModel: t.upstreamModel,
      url: t.url,
      headers: t.headers,
      providerMaxOutputTokens: t.providerMaxOutputTokens,
