...
import { stepOverrides, type ServiceStep } from "@areelai/model-services";
...
import { startFakeUpstream, type FakeUpstream, type UpstreamBehavior } from "@areelai/test-support";
...
async function boot(definition: unknown, script: UpstreamBehavior[]): Promise<Harness> {
  ...
  const { boot: bootContainer } = await import("../src/composition/container.js");
