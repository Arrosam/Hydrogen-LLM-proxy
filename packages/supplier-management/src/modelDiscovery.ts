import { buildHeaders, modelsUrl, type UpstreamProvider } from "./endpoints.js";
import { familyForProviderType } from "@areelai/wire-format";
import type { EgressProxy } from "./egressProxy.js";

/** What a model-list GET returns; structurally satisfied by model-services' transport. */
...
/** The slice of an HTTP transport model discovery needs (a plain GET). The
 * gateway injects model-services' guarded transport; an adopter may pass any
 * fetcher that honours the timeout and, when set, the proxy. */
export interface ModelListTransport {
  getJson(url: string, headers: Record<string, string>, opts: { timeoutMs: number; proxy?: EgressProxy | null }): Promise<ModelListResponse>;
}
