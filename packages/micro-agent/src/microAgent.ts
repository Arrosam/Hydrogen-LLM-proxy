import { ModelService, type InvokeOptions, type ServiceDeps } from "@areelai/model-services";
import { mergeOverrides, type RequestOverrides } from "@areelai/wire-format";
...
import { addUsage, ZERO_USAGE, type Usage } from "@areelai/wire-format";
import { callService, countAttempts, requestPayload, responsePayload, type ServiceCall } from "@areelai/model-services";
export { countAttempts, type ServiceCall } from "@areelai/model-services";
import type { ServiceSteps, ResolveResult, ServiceResolver, ProgressSink } from "@areelai/model-services";
import { stageOverrides, type AgentDef } from "./definition.js";
import { audioHash, transcribeAudio } from "./asr.js";
import {
  buildOcrRequest,
  buildStageRequest,
  collectImages,
...
import { imageHash, type OcrCacheStore } from "./ocrCache.js";
import type { AttemptFailure, AttemptResult } from "@areelai/model-services";
import type { Invocation, InvokeValue, StreamInvocation } from "@areelai/model-services";

/** How deep nested Micro Agents may reference one another before we stop. */
const MAX_AGENT_DEPTH = 8;

export type { ResolveResult, ServiceResolver };
