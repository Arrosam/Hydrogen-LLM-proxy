...
import { AnthropicRequest, AnthropicResponse, OpenAICompletionRequest, OpenAIResponsesRequest, parseStream, serializeStream } from "@areelai/wire-format";
import { collectStream, fabricateStream, newAccumulator, parseSSE, tapStream, type StreamEvent } from "@areelai/wire-format";
...
it("treats HTTP 200 Responses failures as retryable failures", async () => {
  ...
  const result = await sendBuffered(OpenAIResponsesRequest.parse({ input: [], stream: false }), transport, { ...target, url: "http://unused", headers: {}, timeoutMs: 1000 });
