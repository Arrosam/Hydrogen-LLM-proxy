...
import { resolveProxyHost } from "../src/upstream/egress/proxyHost.js";
import { proxyKey, proxyLabel, redactProxy, type EgressProxy } from "@areelai/supplier-management";
import { UpstreamUrlError } from "../src/upstream/ssrf.js";

const SRC = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "../src");
// Every package that reaches a provider through the transport, scanned as one
// tree: a call site in the gateway or the agent package must carry the proxy
// exactly as one here must.
const ROOTS: Record<string, string> = {
  "model-services": SRC,
  "micro-agent": path.resolve(SRC, "../../micro-agent/src"),
  "supplier-management": path.resolve(SRC, "../../supplier-management/src"),
  gateway: path.resolve(SRC, "../../../apps/gateway/src"),
};
...
describe("ST: every provider-addressed request carries the proxy", () => {
  ...
  const EXEMPT = new Map([
    ["model-services/fileFetch.ts", "its URL comes from a client request body; proxying it would let a token holder skip address validation"],
    ["model-services/upstream/client.ts", "this IS the transport; it selects the dispatcher rather than passing a proxy to itself"],
  ]);

  /** Every .ts under a package source root. */
  ...
  it("no transport call reaches a provider without one", () => {
    ...
    const files = Object.entries(ROOTS).flatMap(([pkg, root]) => walk(root).map((file) => [pkg, root, file] as const));
    for (const [pkg, root, file] of files) {
      const rel = `${pkg}/${path.relative(root, file).split(path.sep).join("/")}`;
  ...
  it("the exemptions are real files, so a rename cannot silently widen them", () => {
    for (const rel of EXEMPT.keys()) {
      ...
      expect(fs.existsSync(path.join(ROOTS[pkg], ...rest)), `${rel} is exempted but does not exist`).toBe(true);
    }
  });

  it("the file pre-pass stays direct — its URL is client-supplied", () => {
    const src = code(fs.readFileSync(path.join(SRC, "fileFetch.ts"), "utf8"));
    expect(src).toContain("getStream");
    expect(src).not.toMatch(/\bproxy\b/);
  });

  it("both SendTarget literals in modelService carry the proxy", () => {
    const src = code(fs.readFileSync(path.join(SRC, "modelService.ts"), "utf8"));
  ...
  it("the provider test builds an UpstreamProvider that carries a proxy", () => {
    const src = code(fs.readFileSync(path.join(ROOTS.gateway, "transport/adminRoutes.ts"), "utf8"));
