...
import type { Family, RequestOverrides } from "@areelai/wire-format";
import type { Request } from "@areelai/wire-format";
import { inlineUrlFiles, needsUrlFileInlining } from "./fileFetch.js";
import { fabricateStream } from "@areelai/wire-format";
import type { SendTarget, Transport } from "./upstream/transport.js";
import type { Catalog } from "@areelai/supplier-management";
import { runSteps } from "./steps.js";
import { stepOverrides, type ServiceStep, type ServiceSteps } from "./definition.js";
import type { Invocation, InvokeValue, StreamInvocation, StreamValue } from "./outcome.js";
import type { ProgressSink } from "./progress.js";
import { relayStream, sendBuffered } from "./upstream/roundtrip.js";
...
export interface InvokeOptions {
  hosted?: import("./hostedToolLoop.js").HostedContext;
...
export class ModelService {
  ...
  async invoke(request: Request, overrides?: RequestOverrides, opts: InvokeOptions = {}): Promise<Invocation> {
    ...
    const { result, path } = await runSteps<InvokeValue>(this.def, async (step, stepIndex) => {
      ...
      const sent = await sendBuffered(egress, this.deps.transport, target);
      if (!sent.ok) {
  ...
  async stream(request: Request, overrides?: RequestOverrides, opts: InvokeOptions = {}): Promise<StreamInvocation> {
    ...
    const { result, path } = await runSteps<StreamValue>(this.def, async (step, stepIndex) => {
      ...
      const sent = await relayStream(egress, this.deps.transport, target);
      if (!sent.ok) {
        prog?.record("llm", "llm.receive", `upstream returned ${sent.status}: ${sent.message}`, { status: sent.status });
        return { ok: false, status: sent.status, kind: sent.kind, message: sent.message, errorBody: sent.body };
      }
