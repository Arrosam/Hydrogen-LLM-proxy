...
import { parseStream, serializeStream } from "../src/format/index.js";
import { collectStream, fabricateStream, withoutReasoning, type ResponseData, type StreamEvent } from "../src/ir/stream.js";
import type { ReasoningPart } from "../src/ir/content.js";
...
describe("cross-family output never leaks the payload", () => {
  ...
  it("wraps it in replay metadata for both OpenAI client families", async () => {
    const { data } = await collectStream(parseStream("anthropic", frames(...REDACTED_ONLY)));
    const { buildResponse } = await import("../src/format/index.js");
  ...
  it("is not replayed into an OpenAI-family request body either", async () => {
    const { data } = await collectStream(parseStream("anthropic", frames(...REDACTED_WITH_TOOL)));
    const { AnthropicRequest, OpenAICompletionRequest, OpenAIResponsesRequest } = await import("../src/format/index.js");
...
describe("round trip through an OpenAI Responses client", () => {
  ...
  it("restores the original Anthropic block after the client replays it", async () => {
    const { buildResponse, AnthropicRequest, OpenAIResponsesRequest } = await import("../src/format/index.js");
  ...
  it("does not forward the envelope to a real Responses upstream", async () => {
    const { OpenAIResponsesRequest } = await import("../src/format/index.js");
    const { encodeRedacted } = await import("../src/format/reasoningBridge.js");
  ...
  it("degrades to dropping the block when the envelope is unreadable", async () => {
    const { OpenAIResponsesRequest, AnthropicRequest } = await import("../src/format/index.js");
    const req = OpenAIResponsesRequest.parse({
      model: "svc",
      input: [
        { role: "user", content: [{ type: "input_text", text: "hi" }] },
